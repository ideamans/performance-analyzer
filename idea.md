...
**「実際にタグを全部止めたら指標はどうなるか」を最短で見る**ための唯一の実験系。
他のアナライザが観測（1回のロードからコストを帰属させる）なのに対し、これは
ベースラインと遮断後を実測して引き算する。

### 狙いと割り切り

用途は「ダイエット提案の**天井**を1行で言えるようにする」こと。
厳密な予測ではなく**参考値（上限）**と割り切り、実装も最小にする。

| 論点 | 採用 | 理由 |
|---|---|---|
| 遮断方法 | Lighthouse `blockedUrlPatterns`（→ `Network.setBlockedURLs`） | 実装が数行。追加依存なし |
| 応答の中身 | **空200にしない**（失敗させる） | CDP `Fetch.fulfillRequest` は忠実だが実装コスト大。将来オプション |
| 試行回数 | 既定 片側1回 | 速度優先。ノイズ床の推定はしない |
| ページ破壊 | **許容**（検出しない） | 詳細検証は別途行う前提 |
| 粒度 | 全タグ一括 | ベンダー単位・GTMのみ・上位N累積は将来 |

### 遮断対象

ベースラインを分類し `kind='tag'` のホストを収集（§4.1 の定義をそのまま再利用）。
つまり CDN ライブラリは遮断せず、Google Fonts は遮断する。
パターンは観測ホストの `*://host/*` に加え、`*://*.<eTLD+1>/*` も入れて
2回目のロードで現れる兄弟ホストも捕まえる。

### 実装上の罠（実測で判明）

- 遮断されたリクエストは**トレースから消えない**。`statusCode: -1` / 0バイト /
  数msで終了、という失敗リクエストとして残る。生のリクエスト数を見ると
  「遮断が効いていない」と誤読する。→ 成功リクエストのみ計数し `failedRequests` に分離。
- 実測ではリクエスト数の減少幅（例 243→72）が失敗リクエスト数（25）を大きく上回る。
  ローダー（gtm.js 等）を止めると**子タグがそもそも発生しない**連鎖効果によるもの。
  つまり「GTMだけ遮断」でも実質的に大半のタグが消える。

### 出力の読み方

`delta` は `blocked - baseline` なので**負が改善**（`performanceScore` のみ正が改善）。
`tagRequests` が 0 になっていることが遮断成功のサニティチェックになる。
LCP は破壊によりどちらにも動きうるので、LCP の delta を主張する前に LCP 要素が
同じか確認する。
