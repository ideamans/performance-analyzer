# performance-analyzer 設計メモ（idea.md）

## 0. このツールの狙い（プロダクトの目的）

**「自社サイトは遅い」と感じている人が、速い競合サイトと並べて
"どこに無駄・妨害があるのか" を自覚するためのベンチマーク／反省ツール。**

ありがちな気づきを定量的に突きつけることをゴールにする:

- 「速いサイトは LCP までに読むデータ量・処理がこんなに少ない」
- 「自社サイトは LCP 到達までに無駄な処理・妨害が多い／ネットワークが遅い」
- 「速いサイトはサードパーティタグが少ない。自社は多すぎる」
- 「自社の Google Tag Manager はタグ設定が盛りすぎ」

→ ツールは「速いサイト」「遅い（自社）サイト」それぞれに対して **同じ分析を
実行し、レポートを並べて見比べられる** ようにする。
（※ 自動 diff の `compare` 機能は作らない。各サイトのレポートを人が見比べる。）
...
```
  ...
  URL ──► [Capture] ──►   │  成果物（artifacts）         │
        Lighthouse(1回)   │   - trace.json               │
            ...
            ┌────────────────────────────┼────────────────────────────┐
            ▼                            ▼                            ▼
  [Analyzer 1: LCP到達分析]    [Analyzer 2: サードパーティタグ]   [Analyzer 3: GTM詳細]
   何が起き/邪魔し/遅いか        分量・CPU消費・ベンダー内訳        コンテナ取得→設定解析
            │                            │                            │
            └──────────► 各 Analyzer が独立したレポートを出力 ◄────────┘

  [Analyzer 4(将来): 画像分析]  読み込みが遅い/多すぎる画像 等
```
...
- CLI イメージ（compare は無し。capture と各 analyze）:
  ```
  performance-analyzer capture <url> --out ./runs/example
  performance-analyzer analyze lcp          ./runs/example
  performance-analyzer analyze third-party  ./runs/example
  performance-analyzer analyze gtm          ./runs/example
