...
- **LCP 到達分析（`lcp`）** — FCP/LCP までに「何が起き／何が妨害し／どこが遅いか」
  ...
  - **CRP リソースごとの取得分解**（HTML/レンダーブロッキングCSS・JS/LCP画像を、DNS/接続/TLS のレイテンシー・TTFB・ダウンロード・サイズ・新規接続有無・主因で分解）— どのクリティカルリソースの取得が遅いかを即特定
...
- **GTM 詳細分析（`gtm`）** — 「GTM がなぜ肥大化しているか／何に使っているか」を**客観的な計数**で外観把握
  ...
  - **タグの MECE 分類**（発火タグ / 制御タグ＝オートイベントリスナ等 / 停止中タグ、合計＝総タグ数）— 「実際に動くタグは何個か」を客観把握
